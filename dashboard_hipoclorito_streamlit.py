...
dados_mapa_final['NOME_MUNICIPIO'] = dados_mapa_final['NOME_MUNICIPIO'].apply(lambda x: x.strip().upper())

#geojson_str = dados_mapa_final.to_json()

map_fig = folium.Map(location=[-30.452349861219243, -53.55320517512141], zoom_start=7)
choropleth = folium.Choropleth(
    geo_data=dados_mapa_final,
    name='choropleth',
    data=dados_mapa_final,
    ...
    fill_opacity=1,  # Maintain fill opacity
    line_opacity=0.55,  # Reduce line opacity
...
def style_function(feature):
    return {
        ...
        'color': 'transparent',  # Make outline color transparent
        'weight': 0,  # Remove outline weight
    }

# Add hover information (Tooltip) to each GeoJson feature
folium.GeoJson(
    dados_mapa_final,
    tooltip=folium.GeoJsonTooltip(
        fields=['NOME_MUNICIPIO', 'CRS', 'Quantidade de Frascos 50mL'],
        aliases=['Município', 'CRS', 'Quantidade de Frascos 50mL']
    ),style_function=style_function
).add_to(map_fig)
# Set the order of layers
map_fig.add_child(choropleth)
...
tabela_df_stilished = tabela_df.style.background_gradient(cmap='Purples', subset=['Quantidade de Frascos Distribuídos', 'Número de Pedidos'])
...
with col2:
    folium_static(map_fig,width=1000, height=675)
